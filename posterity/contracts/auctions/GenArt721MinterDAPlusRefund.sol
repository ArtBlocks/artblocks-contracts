...
contract GenArt721MinterDAPlusRefund {
    ...
    function withdrawExcess(uint256 _projectId) public payable returns (bool) {
        require(
            !isAuctionLive(_projectId),
            "cannot withdraw until after auction"
        );
        require(
            !addressClaimedRefund[msg.sender][_projectId],
        ...
        if (auctionFinalPrice[_projectId] > 0) {
            ...
            uint256 finalPrice = artblocksContract
    ...
    function splitAuctionFunds(uint256 _projectId)
        public
        payable
        returns (bool)
    {
        require(
            artblocksContract.isWhitelisted(msg.sender),
            "can only be triggered by admin"
        );
        require(
            !isAuctionLive(_projectId),
            "cannot trigger while auction is live"
        );
        ...
        returns (uint256 _tokenId)
    {
        if (
            ...
            if (isAuctionLive(_projectId)) {
                ...
                require(
                    totalIterations < projectMaxIterations,
                    "max number of auctioned iterations met"
                );
    ...
    function _splitFundsETHAuction(uint256 _projectId) internal {
        require(
            !projectSplitAuctionFunds[_projectId],
            "Funds have already been split for this project"
        );
        require(
            !isAuctionLive(_projectId),
            "cannot split funds during an auction"
        );

        uint256 totalFundsToSplit;
        if (auctionFinalPrice[_projectId] > 0) {
            totalFundsToSplit = auctionFinalPrice[_projectId].mul(
                auctionIterationCounter[_projectId]
            );
        } else {
            totalFundsToSplit = getPrice(_projectId).mul(
                auctionIterationCounter[_projectId]
            );
        }
    ...
    function isAuctionLive(uint256 _projectId) public view returns (bool) {
        if (block.timestamp < auctionTimestamp[_projectId]) {
            ...
            auctionIterationCounter[_projectId] ==
            auctionedIterations[_projectId]
        ) {
            return false;
        } else {
            return
    ...
    function getRefundAmount(address _address, uint256 _projectId)
        public
        view
        returns (uint256)
    {
