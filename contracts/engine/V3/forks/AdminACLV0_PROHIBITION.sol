 ...
 */
contract AdminACLV0 is IAdminACLV0_PROHIBITION, ERC165 {
    string public AdminACLType = "AdminACLV0";

    /// superAdmin is the only address that passes any and all ACL checks
    address public superAdmin;

    /// approvedCallers is a mapping of contracts -> selectors -> addresses
    /// other than the superAdmin, these addresses are the only ones that can call
    /// the function with the given selector on the given contract
    mapping(address => mapping(bytes4 => address)) public approvedCallers;
    ...
    function getApprovedCaller(
        ...
        return approvedCallers[_contract][_selector];
    }
    ...
    function updateApprovedCaller(
        ...
        require(msg.sender == superAdmin, "Only superAdmin");
        approvedCallers[_contract][_selector] = _caller;
        emit ApprovedCallerUpdated(_contract, _selector, _caller);
    }
    ...
    function allowed(
        ...
        return superAdmin == _sender || approvedCallers[_contract][_selector] == _sender;
    }
