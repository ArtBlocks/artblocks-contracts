...
interface IGenArt721CoreV2WithEvents {
    /**
     ...
     */
    event ProjectUpdated(uint256 indexed _projectId, string _update);

    /**
     * @notice Project ID `_projectId` updated script at index `_scriptId`.
     */
    event ProjectScriptUpdatedAtIndex(
        uint256 indexed _projectId,
        uint256 indexed _scriptId
    );

    /**
     * @notice Platform updated on field `_field`.
     */
    event PlatformUpdated(string _field);

    /**
     * @notice Allowlist updated with bool `_isApproved` on address `_address`.
     */
    event AllowlistUpdated(bool _isApproved, address _address);

    /**
     * @notice Mint aAllowlist updated with bool `_isApproved` on address `_address`.
     */
    event MintAllowlistUpdated(bool _isApproved, address _address);
