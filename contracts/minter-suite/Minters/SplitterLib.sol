 ...
 */
library SplitterLib {
    bytes32 constant SPLITTER_LIB_STORAGE_POSITION =
        keccak256("splitter-lib.storage");
    ...
    /// @return storageStruct The MinterSEAV0LibStorage struct.
    function s()
        internal
        pure
        returns (SplitterLibStorage storage storageStruct)
    {
        bytes32 position = SPLITTER_LIB_STORAGE_POSITION;
        assembly {
            storageStruct.slot := position
        }
    }

    function init(address genArt721Address) internal {
        // set state variable isEngine
        s().isEngine = _getV3CoreIsEngine(genArt721Address);
    }
    ...
    function isEngine() public view returns (bool) {
        return s().isEngine;
    }
