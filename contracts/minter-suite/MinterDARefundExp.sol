 ...
 */
contract MinterDARefundExpV0 is ReentrancyGuard, IFilteredMinterV0 {
    ...
    event ProjectRefundsAvailable(
        ...
        uint256 _refundableInvocations
    );

    /// Auction has ended for project `projectId`. Refunds now available.
    event AuctionEnded(uint256 indexed projectId, uint256 minPurchasePrice);
     ...
     * @param _projectId Project ID to set auction details for.
     * @dev this does not reset parameters related to refunds
     */
     ...
     * price.
     */
    function adminLockRefundableBasePrice(uint256 _projectId)
        external
        onlyCoreWhitelisted
    {
