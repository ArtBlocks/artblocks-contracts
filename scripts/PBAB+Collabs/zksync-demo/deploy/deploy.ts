...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  const wallet = new Wallet(process.env.TESTNET_PRIVATE_KEY as string);
  ...
  // uncomment if you need L2 funds in your wallet
  ...
  // console.log('1');
  // console.log(randomizerContract);
  ...
  // const deploymentFee = await deployer.estimateDeployFee(greeterArtifact, [greeting]);
  // const greeterContract = await deployer.deploy(greeterArtifact, [greeting]);

  //   const parsedFee = ethers.utils.formatEther(deploymentFee.toString());
  //   console.log(`The deployment is estimated to cost ${parsedFee} ETH`);

  //     console.log(
  //   `${greeterArtifact.contractName} was deployed to ${greeterContract.address}`
  // );

    //   const deploymentFee = await deployer.estimateDeployFee(coreArtifact, [
    //   tokenName,
    // tokenTicker,
    // randomizerAddress,
    // startingProjectId
    // ]);
    console.log('trying...')
    const factoryDeps = await deployer.extractFactoryDeps(coreArtifact)
    const factory = new zk.ContractFactory(coreArtifact.abi, coreArtifact.bytecode, deployer.zkWallet);
          ...
          deployTx.from = deployer.zkWallet.address;
          console.log('gas costs: ', deployer.zkWallet.provider.estimateGas(deployTx))
          console.log('done!')
  ...
  console.log(err)
  console.log('QWERTY: ', err.transaction.customData.ergsPerPubdata.toString());
}
