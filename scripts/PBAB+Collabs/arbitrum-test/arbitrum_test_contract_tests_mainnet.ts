...
import { abi } from "../../../artifacts/contracts/PBAB+Collabs/art-blocks-x-pace/GenArt721CoreV2_ArtBlocksXPace.sol/GenArt721CoreV2_ArtBlocksXPace.json";
import { abi as MinterFilterABI } from "../../../artifacts/contracts/minter-suite/MinterFilterV0.sol/MinterFilterV0.json";
import { abi as MinterSetPriceABI } from "../../../artifacts/contracts/minter-suite/MinterSetPriceV1.sol/MinterSetPriceV1.json";

const CORE_CONTRACT_ADDRESS = "0xFFF67B5dEee6a34D170b84Ba1bD3c87769f2Ccc1";
const MINTER_FILTER_V0_CONTRACT_ADDRESS =
  "0xF1179fa851899A22321A07414Dac11a1935Fd266";
const MINTER_SET_PRICE_CONTRACT_ADDRESS =
  "0xFaF9d1A787b420f34b566edAA0b7B0b8810fFB4A";
const PROJECT_ID_TO_ADD = 0;
...
async function runSkulTest(
  coreContract,
  minterFilter,
  minterSetPrice,
  deployer
) {
  // const addProjectRes = await coreContract.addProject(
  //   "TESTPROJECT",
  //   deployer.address,
  //   PROJECT_ID_TO_ADD
  // );
  ...
  //   196
  // );
  // console.log("succesfully updated project max invocations");
  ...
  // );
  // await delay(20000);
  // console.log("succesfully added project script");

  // await minterFilter.addApprovedMinter(MINTER_SET_PRICE_CONTRACT_ADDRESS);
  // await minterFilter.setMinterForProject(
  //   PROJECT_ID_TO_ADD,
  //   MINTER_SET_PRICE_CONTRACT_ADDRESS
  // );
  // await delay(20000);
  // await minterSetPrice.updatePricePerTokenInWei(PROJECT_ID_TO_ADD, 0);
  // await delay(20000);
  await minterSetPrice.purchase(PROJECT_ID_TO_ADD);
}
...
async function main() {
  ...
  const minterFilter = new ethers.Contract(
    MINTER_FILTER_V0_CONTRACT_ADDRESS,
    MinterFilterABI,
    deployer
  );
  const minterSetPrice = new ethers.Contract(
    MINTER_SET_PRICE_CONTRACT_ADDRESS,
    MinterSetPriceABI,
    deployer
  );
  await runSkulTest(coreContract, minterFilter, minterSetPrice, deployer);
}
