...
// flagship
import { GenArt721CoreV1__factory } from "../contracts/factories/GenArt721CoreV1__factory";
// minter suite
...
/**
 * This script was created to deploy flagship contracts to the goerli
...
// CONFIG BEGINS HERE
//////////////////////////////////////////////////////////////////////////////
const tokenName = "Art Blocks Dev (Goerli)";
const tokenTicker = "BLOCKS_DEV_GOERLI";
const transferAddress = "0x00000000000730639587DCC1541C04406fFe4B7D"; // note: cannot transfer Admin of V1 core
...
async function main() {
  ...
  console.log(`MinterSetPrice V1 deployed at ${minterSetPrice.address}`);
  // set price ERC20 V1
  ...
  console.log(
    `MinterSetPrice ERC20 V1 deployed at ${minterSetPriceERC20.address}`
  );
  // DA Lin V1
  ...
  console.log(`Minter DA Lin V1 deployed at ${minterDALin.address}`);
  // DA Exp V1
  ...
  console.log(`Minter DA Exp V1 deployed at ${minterDAExp.address}`);
  ...
  await genArt721Core
    .connect(deployer)
    .updateArtblocksAddress(artblocksAddress);
  ...
  // Allowlist new owner.
  await genArt721Core.connect(deployer).addWhitelisted(transferAddress);
  console.log(`Allowlisted Core contract access for: ${transferAddress}.`);
  delay(EXTRA_DELAY_BETWEEN_TX);

  // Cannot transfer Core contract admin to new owner on V1 contract.
