...
async function main() {
  ...
  await coreToken.connect(deployer).toggleProjectIsActive(0, { gasLimit: 500000 });

  await coreToken.connect(deployer).addMintWhitelisted(minterFilter.address, { gasLimit: 500000 });

  await coreToken.connect(deployer).updateProjectMaxInvocations(0, 15, { gasLimit: 500000 });

  await coreToken.connect(deployer).toggleProjectIsPaused(0, { gasLimit: 500000 });
