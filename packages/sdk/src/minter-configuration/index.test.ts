...
type MockArtBlocksClientContext = ArtBlocksClientContext & {
  publicClientResolver: PublicClientResolver;
  publicClient: PublicClient; // Keep for direct test access
};

function createMockArtBlocksClientContext(): MockArtBlocksClientContext {
  ...
  return {
    ...
    publicClientResolver: () => mockPublicClient,
    graphqlClient: {
  ...
  } as unknown as MockArtBlocksClientContext;
}

describe("generateProjectMinterConfigurationForms", () => {
  let artblocksClientContext: MockArtBlocksClientContext =
    createMockArtBlocksClientContext();
  ...
  describe("generateProjectMinterConfigurationForms", () => {
    ...
    it("returns just the minter selection form if the project has no minter specified", async () => {
      ...
      const { minter_configuration, ...mockProjectData } =
        testResponse.projects_metadata[0];
      (
        artblocksClientContext.graphqlClient.request as jest.Mock
      ).mockResolvedValueOnce({
        projects_metadata: [mockProjectData],
      });
    ...
    it("returns only the minter selection form if the project minter has no configuration schema", async () => {
      ...
      mockProjectData.projects_metadata[0].minter_configuration.minter.type.project_configuration_schema =
        {} as any;
      (
        artblocksClientContext.graphqlClient.request as jest.Mock
