...
/**
 * General Integration tests for V4 core.
 */
    ...
    describe("artblocksAddress", function () {
      if (coreContractName.includes("GenArt721CoreV4_Engine")) {
        it("always passes, non-relevant", async function () {
          const config = await loadFixture(_beforeEach);
          // This test is non-relevant for Engine variant V4 contracts.
          expect(true);
        });
      } else {
    ...
    describe("coreType", function () {
      it("returns expected value", async function () {
        ...
        const coreType = await config.genArt721Core
          .connect(config.accounts.deployer)
          .coreType();
        // coreType is same for GenArt721CoreV4 & GenArt721CoreV4_Explorations
        expect(coreType).to.be.equal("GenArt721CoreV4");
      });
    });
    ...
    describe("setTokenHash_8PT", function () {
      it("does not allow non-randomizer to call", async function () {
        ...
        await expectRevert(
          config.genArt721Core
            .connect(config.accounts.artist)
            .setTokenHash_8PT(1, ethers.constants.MaxInt256),
      ...
      it("does allow randomizer to call, and updates token hash", async function () {
        ...
        await config.minter
          .connect(config.accounts.artist)
          .purchase(config.projectZero);
        expect(await config.genArt721Core.tokenIdToHash(1)).to.not.be.equal(
      ...
      it("does not allow randomizer to call once a token hash has been set", async function () {
        const config = await loadFixture(_beforeEach);
        // ensure token hash is initially zero
        expect(await config.genArt721Core.tokenIdToHash(1)).to.be.equal(
          ethers.constants.HashZero
        );
        ...
        await mockRandomizer.actuallyAssignTokenHash(1);
        // expect revert when attempting to overwrite the token hash
        await expectRevert(
          mockRandomizer.actuallyAssignTokenHash(1),
      ...
      it("does not allow randomizer to set token hash seed to zero", async function () {
        ...
        await expectRevert(
          mockRandomizer.actuallyAssignZeroTokenHash(1),
          "No zero hash seed"
        );
      });
