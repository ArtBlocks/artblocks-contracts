...
describe("SRHooks Integration Tests", function () {
  ...
  describe("Delegation V2 Integration", function () {
    it("allows delegate.xyz V2 delegate to update token state [@skip-on-coverage]", async function () {
      ...
      await expect(
        updateImage(config.srHooksProxy, 0, imageData, 0, config.accounts.user2) // delegate calling!
      ).to.not.be.reverted;
    ...
    it("reverts when delegate has wrong rights", async function () {
      ...
      await expect(
        updateImage(config.srHooksProxy, 0, imageData, 0, config.accounts.user2)
      ).to.be.reverted;
    });
    ...
    it("does not allow delegation for wrong token", async function () {
      ...
      await expect(
        updateImage(
          ...
          0,
          config.accounts.additional
        ) // additional is NOT owner or delegate of token 1
      ).to.be.reverted;
    });
  });
  ...
  describe("PMP Augmentation Hook Integration", function () {
    ...
    it("correctly converts all send states to string enum names", async function () {
      ...
      // Test Neutral send state (default after just adding image)
    ...
    it("reflects active slot data after slot change", async function () {
      ...
      await changeActiveSlot(config.srHooksProxy, 0, 0, config.accounts.user);
