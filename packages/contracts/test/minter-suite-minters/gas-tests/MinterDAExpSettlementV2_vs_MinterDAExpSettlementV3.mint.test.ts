...
describe(`${CORE_NAME} Gas Tests`, async function () {
  ...
  async function _beforeEachMinterSuiteLegacy() {
    ...
    config.minter = await deployAndGet(config, "MinterDAExpSettlementV2", [
  ...
  async function _beforeEachMinterSuiteV2() {
    ...
    config.minter = await deployAndGet(config, "MinterDAExpSettlementV3", [
      config.minterFilter.address,
    ]);
    await config.minterFilter
      .connect(config.accounts.deployer)
  ...
  describe("LEGACY mint gas measurement", function () {
    it("test gas cost of mint on LEGACY MinterDAExpSettlementV2 [ @skip-on-coverage ]", async function () {
  ...
  describe("SHARED MINTER SUITE mint gas measurement", function () {
    it("test gas cost of mint on SHARED MinterDAExpSettlementV3 [ @skip-on-coverage ]", async function () {
