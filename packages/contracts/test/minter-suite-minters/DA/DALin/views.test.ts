...
import {
  ...
  configureProjectZeroAuctionAndAdvanceToEnd,
  ...
  describe(`${TARGET_MINTER_NAME} Views w/ core ${params.core}`, async function () {
    ...
    describe("getPriceInfo", async function () {
      ...
      it("returns correct price mid-auction", async function () {
        ...
        // advance exactly one half way through auction and check that price is exactly halved
        await ethers.provider.send("evm_mine", [
          config.startTime + (config.endTime - config.startTime) / 2,
        ]);
        priceInfo = await config.minter.getPriceInfo(
          config.projectZero,
          config.genArt721Core.address
        );
        const targetHalfPrice = config.startingPrice
          .add(config.basePrice)
          .div(2);
        expect(priceInfo.isConfigured).to.equal(true);
        expect(priceInfo.tokenPriceInWei).to.equal(targetHalfPrice);
        // advance exactly 75% through auction, and check that price is down exactly 75%
        await ethers.provider.send("evm_mine", [
          config.startTime + (config.endTime - config.startTime) * 0.75,
        ]);
        priceInfo = await config.minter.getPriceInfo(
          config.projectZero,
          config.genArt721Core.address
        );
        const targetPriceAfterSeventyFivePercent = targetHalfPrice
          .add(config.basePrice)
          .div(2);
        expect(priceInfo.isConfigured).to.equal(true);
        expect(priceInfo.tokenPriceInWei).to.equal(
          targetPriceAfterSeventyFivePercent
        );
      });

      it("returns correct price after auction", async function () {
        const config = await loadFixture(_beforeEach);
        await configureProjectZeroAuctionAndAdvanceToEnd(config);
        const priceInfo = await config.minter.getPriceInfo(
          config.projectZero,
          config.genArt721Core.address
        );
    ...
    describe("minimumAuctionLengthSeconds", async function () {
      it("returns correct initial value", async function () {
        const config = await loadFixture(_beforeEach);
        const minimumAuctionLengthSeconds =
          await config.minter.minimumAuctionLengthSeconds();
        expect(minimumAuctionLengthSeconds).to.equal(600);
      });
    });
    ...
    describe("projectAuctionParameters", async function () {
      it("returns correct unconfigured values", async function () {
        ...
        expect(projectAuctionParameters.timestampEnd).to.equal(0);
      ...
      it("returns correct configured values", async function () {
        ...
        expect(projectAuctionParameters.timestampEnd).to.equal(config.endTime);
