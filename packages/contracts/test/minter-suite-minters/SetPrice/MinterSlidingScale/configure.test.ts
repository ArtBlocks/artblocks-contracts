...
runForEach.forEach((params) => {
  describe(`${TARGET_MINTER_NAME} Configure w/ core ${params.core}`, async function () {
    ...
    describe("updatePricePerTokenInWei", async function () {
      it("enforces price update as minimum", async function () {
        const config = await loadFixture(_beforeEach);
        // artist sets minimum price
        await config.minter
          .connect(config.accounts.artist)
          .updatePricePerTokenInWei(
            config.projectZero,
        ...
        // cannot purchase token below minimum price
        await expectRevert(
          config.minter
        ...
        // can purchase token at minimum price
        await config.minter
          .connect(config.accounts.user)
          .purchase(config.projectZero, config.genArt721Core.address, {
            value: config.pricePerTokenInWei,
          });
        // can purchase token above minimum price
        await config.minter
          .connect(config.accounts.user)
          .purchase(config.projectZero, config.genArt721Core.address, {
      ...
      it("enforces price update only on desired project", async function () {
        ...
        // artist sets higher minimum price of project one
        await config.minter
          .connect(config.accounts.artist)
          .updatePricePerTokenInWei(
            config.projectOne,
