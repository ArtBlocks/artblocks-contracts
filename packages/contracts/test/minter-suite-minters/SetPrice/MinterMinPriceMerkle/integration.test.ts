    ...
    describe("purchase", async function () {
      ...
      describe("payment splitting", async function () {
        ...
        it("sends all payment to render provider", async function () {
          ...
          // update render provider address to user2 address
          ...
          // record user2 balance before purchase
          ...
          // record user2 balance after purchase
          const user2BalanceAfter = await ethers.provider.getBalance(
            config.accounts.user2.address
          );
          // expect user balance to be equal to price paid
          expect(user2BalanceAfter).to.equal(
            user2BalanceBefore.add(config.pricePerTokenInWei)
          );
        });
