...
import "./interfaces/v0.8.x/IManifoldCore.sol";
import "./interfaces/v0.8.x/IManifoldCreatorExtensionURI.sol";
...
/**
 * @title Art Blocks ERC-721 core contract, V4.
 * @author Art Blocks Inc. & YUNG WKND LLC
 ...
 */
contract GenArt721CoreV4 is
    ERC721_PackedHashSeed,
    Ownable,
    IGenArt721CoreContractV3,
    IGenArt721CoreContractExposesHashSeed,
    IManifoldCreatorExtensionURI
{
    ...
    string public constant coreVersion = "v4.0.0";
    string public constant coreType = "GenArt721CoreV4";

    /// default base URI to initialize all new project projectBaseURI values to
    string public defaultBaseURI;

    // Maps tokenIds to projectIds because cannot just divide by 1M anymore
    mapping(uint => uint) public tokenIdToProjectIds;
    ...
    function _onlyValidTokenId(uint256 _tokenId) internal view {
        try
            IManifoldCore(
                projects[tokenIdToProjectId(_tokenId)].artistContractAddress
            ).ownerOf(_tokenId)
        returns (address) {
            // Do nothing
        } catch {
    ...
    function mint_Ecf(
        ...
        // Mint to the artist's contract.
        uint256 thisTokenId = IManifoldCore(project.artistContractAddress)
            .mintExtension(_to);

        tokenIdToProjectIds[thisTokenId] = _projectId;
    ...
    function addProject(
        ...
        address payable _artistAddress,
        address _artistContractAddress
    ) external {
        ...
        projects[projectId].artistContractAddress = _artistContractAddress;
    ...
    /**
     * @notice Updates artist contract address of project `_projectId` to be `__artistContractAddress`.
     * @param _projectId Project ID.
     * @param _artistContractAddress New project artist contract address.
     */
    function updateProjectArtistContractAddress(
        uint256 _projectId,
        address _artistContractAddress
    ) external {
        _onlyUnlocked(_projectId);
        ...
        require(
            ...
            "Cannot change artist contract address after invocations have started"
        );
        projects[_projectId].artistContractAddress = _artistContractAddress;
        emit ProjectUpdated(_projectId, FIELD_PROJECT_NAME);
    }
    ...
    function tokenIdToProjectId(
        uint256 _tokenId
    ) public view returns (uint256 _projectId) {
        return tokenIdToProjectIds[_tokenId];
    }
     ...
     * @param _creatorCore Address of creatorCore contract.
     * @param _tokenId Token ID to be queried.
    ...
    function supportsInterface(
        ...
        return
            ...
            interfaceId == type(IManifoldCreatorExtensionURI).interfaceId ||
     ...
     * @dev owner role was called `admin` prior to V4 core contract.
