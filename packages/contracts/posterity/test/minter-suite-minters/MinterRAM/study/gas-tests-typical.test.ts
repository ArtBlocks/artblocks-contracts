...
const iterations = [10000, 10000]; //[10, 20, 50, 100, 200, 1000, 2000]; //

/**
 * Gas tests for RAM minter study contracts.
 * Used when designing for gas efficiency tradeoffs to aide in decision making.
 * Tests are for "typical" use case, where bids are inserted near the minimum
 * bid value, and the heap is full.
 */
iterations.forEach((iteration) => {
  describe(`RAM Minter Gas Tests (${iteration} project size)`, async function () {
    ...
    describe("bid insertion", function () {
      it(`baseline - typical-case-insertion - ${iteration} max-invocations [ @skip-on-coverage ]`, async function () {
        ...
        // insert ${iteration} bids at slot 1
        ...
        // get gas cost to place a bid at slot 1
        // @dev this should remove bid at slot 0, and do a non search for next slot 1
      ...
      it(`study - typical-case-insertion - ${iteration} max-invocations [ @skip-on-coverage ]`, async function () {
        const config = await loadFixture(_beforeEach);
        // set minter to baseline
        await config.minterFilter
          .connect(config.accounts.deployer)
        ...
        // @dev temp don't worry about start time
        // insert ${iteration-2} random value bids, at any of lowest 100 multiples of min bid value
        ...
        // place last bid at near minimum value, simulating a typical case
        await config.minter_study
          .connect(config.accounts.user)
          .createBid(config.projectZero, config.genArt721Core.address, {
            value: config.basePrice?.mul(2),
        ...
        // get gas cost to place a bid near minimum value (simulating a typical case)
        await expectRevert(
          config.minter_study
            .connect(config.accounts.user)
            .createBid(config.projectZero, config.genArt721Core.address, {
