...
// test the following DependencyRegistry implementations:
const dependencyRegistriesToTest = [
  "DependencyRegistryV0",
  "DependencyRegistryV1",
];

interface DependencyRegistryCommonTestContext extends Mocha.Context {
  config: DependencyRegistryCommonTestConfig;
}

interface DependencyRegistryCommonTestConfig extends T_Config {
  dependencyRegistry?:
    | RemoveIndex<DependencyRegistryV0>
    | RemoveIndex<DependencyRegistryV1>;
 ...
 */
for (const dependencyRegistryToTest of dependencyRegistriesToTest) {
  describe(`${dependencyRegistryToTest} Common`, async function () {
    const dependencyType = "p5js@1.0.0";
    const dependencyTypeBytes =
    ...
    async function _beforeEach() {
      let config: DependencyRegistryCommonTestConfig = {
        accounts: await getAccounts(),
      };
      config = (await assignDefaultConstants(
        config
      )) as DependencyRegistryCommonTestConfig;
      ...
      config.dependencyRegistry = await deployAndGet<
        RemoveIndex<DependencyRegistryV0> | RemoveIndex<DependencyRegistryV0>
      >(config, dependencyRegistryToTest);
    ...
    describe("registered dependencies", function () {
      describe("addDependency", function () {
        it("does not allow non-admins to add a dependency", async function (this: DependencyRegistryCommonTestContext) {
        ...
        it("does not allow a dependency to be added without exactly one @ symbol", async function (this: DependencyRegistryCommonTestContext) {
          const config = await loadFixture(_beforeEach);
          // deployer cannot update
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to add a dependency", async function (this: DependencyRegistryCommonTestContext) {
          const config = await loadFixture(_beforeEach);
          // admin can update
          await expect(
            config.dependencyRegistry
      ...
      describe("removeDependency", function () {
        it("does not allow non-admins to remove a dependency", async function (this: DependencyRegistryCommonTestContext) {
          const config = await loadFixture(_beforeEach);
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
            .addDependency(
        ...
        it("does not allow removal of a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          const config = await loadFixture(_beforeEach);
          await expectRevert(
            config.dependencyRegistry
              .connect(config.accounts.deployer)
        ...
        it("does not allow removal of a dependency with additional CDNs, repositories, or scripts", async function (this: DependencyRegistryCommonTestContext) {
          const config = await loadFixture(_beforeEach);
        ...
        it("allows admin to remove a dependency", async function (this: DependencyRegistryCommonTestContext) {
          const config = await loadFixture(_beforeEach);
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
            .addDependency(
      ...
      describe("update", function () {
        ...
        describe("updateDependencyPreferredCDN", function () {
          it("does not allow non-admins to update preferred cdn", async function (this: DependencyRegistryCommonTestContext) {
          ...
          it("does not allow updating preferred cdn for a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
            // get config from beforeEach
            const config = this.config;
            await expectRevert(
              config.dependencyRegistry
          ...
          it("allows admin to update preferred cdn", async function (this: DependencyRegistryCommonTestContext) {
            // get config from beforeEach
            const config = this.config;
            await config.dependencyRegistry
              .connect(config.accounts.deployer)
        ...
        describe("updateDependencyPreferredRepository", function () {
          it("does not allow non-admins to update preferred repository", async function (this: DependencyRegistryCommonTestContext) {
            // get config from beforeEach
            const config = this.config;
            await expectRevert(
              config.dependencyRegistry
          ...
          it("does not allow updating preferred repository for a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
            // get config from beforeEach
            const config = this.config;
            await expectRevert(
              config.dependencyRegistry
          ...
          it("allows admin to update preferred repository", async function (this: DependencyRegistryCommonTestContext) {
            // get config from beforeEach
            const config = this.config;
            await expect(
              config.dependencyRegistry
        ...
        describe("updateDependencyReferenceWebsite", function () {
          it("does not allow non-admins to update reference website", async function (this: DependencyRegistryCommonTestContext) {
            // get config from beforeEach
            const config = this.config;
            await expectRevert(
              config.dependencyRegistry
          ...
          it("does not allow updating reference website for a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
            // get config from beforeEach
            const config = this.config;
            await expectRevert(
              config.dependencyRegistry
          ...
          it("allows admin to update reference website", async function (this: DependencyRegistryCommonTestContext) {
            // get config from beforeEach
            const config = this.config;
            await expect(
              config.dependencyRegistry
    ...
    describe("dependency scripts", function () {
      ...
      describe("addDependencyScript", function () {
        it("does not allow non-admins to add a script", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow adding a script to a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow adding an empty string as a script", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to add a script", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
      ...
      describe("removeDependencyLastScript", function () {
        it("does not allow non-admins to remove a script", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow removing a script from a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow removing the last script if non-existent", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to remove last script", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          const script = "on-chain script";
      ...
      describe("updateDependencyScript", function () {
        it("does not allow non-admins to update a script", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow updating a script for a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow updating a script that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow updating an empty string as a script", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          const script = "on-chain script";
        ...
        it("allows admin to update a script", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          const script = "on-chain script";
      ...
      describe("views", function () {
        it("getDependencyDetails", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          const script = "on-chain script";
        ...
        it("getDependencyScriptAtIndex", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          const script = "on-chain script";
        ...
        it("getDependencyScriptBytecodeAddressAtIndex", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          const script = "on-chain script";
    ...
    describe("dependency additional cdns", function () {
      ...
      describe("addDependencyAdditionalCDN", function () {
        it("does not allow non-admins to add a cdn", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow adding a cdn for a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow adding an empty string as a cdn", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to add a cdn", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expect(
            config.dependencyRegistry
      ...
      describe("removeDependencyAdditionalCDNAtIndex", function () {
        it("does not allow non-admins to remove a cdn", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow removing a cdn for a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow removing a cdn with out of range index", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to remove a cdn", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
      ...
      describe("updateDependencyAdditionalCDNAtIndex", function () {
        it("does not allow non-admins to update a cdn", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow updating a cdn for a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow updating a cdn with out of range index", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow updating a cdn with empty string", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to update a cdn", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
      ...
      describe("views", function () {
        ...
        it("getDependencyAdditionalCDNAtIndex", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
    ...
    describe("dependency additional repositories", function () {
      ...
      describe("addDependencyAdditionalRepository", function () {
        it("does not allow non-admins to add additional repository", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow adding additional repository for a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow adding empty string as additional repository", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to add additional repository", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expect(
            config.dependencyRegistry
      ...
      describe("removeDependencyAdditionalRepositoryAtIndex", function () {
        it("does not allow non-admins to remove additional repository", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow removing additional repository for a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow removing additional repository at index that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to remove additional repository", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
      ...
      describe("updateDependencyAdditionalRepositoryAtIndex", function () {
        it("does not allow non-admins to update additional repository", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow updating additional repository for a dependency that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow updating additional repository at index that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow updating additional repository to empty string", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to update additional repository", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
      ...
      describe("views", function () {
        ...
        it("getDependencyAdditionalRepositoryAtIndex", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
    ...
    describe("project dependency override", function () {
      ...
      describe("addSupportedCoreContract", function () {
        it("does not allow non-admins to add supported core contract", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow adding supported core contract that already exists", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
        ...
        it("does not allow the zero addresss", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to add supported core contract", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expect(
            config.dependencyRegistry
      ...
      describe("removeSupportedCoreContract", function () {
        it("does not allow non-admins to remove supported core contract", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow removing supported core contract that does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to remove supported core contract", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
      ...
      describe("addProjectDependencyTypeOverride", function () {
        it("does not allow non-admins to add project dependency override", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow adding override that is not a registered dependency", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("does not allow adding override for a project that is not on a supported core contract", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to add project dependency override", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
      ...
      describe("removeProjectDependencyTypeOverride", function () {
        it("does not allow non-admins to remove project dependency override", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("reverts if override does not exist", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
            config.dependencyRegistry
        ...
        it("allows admin to remove project dependency override", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
      ...
      describe("getDependencyTypeForProject", function () {
        it("reverts if core contract is not supported", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await expectRevert(
        ...
        it("reverts if no override and contract does not support projectScriptDetails", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          const {
        ...
        it("returns the overridden value if present", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
        ...
        it("returns the dependency type from the core contract if no override is set", async function (this: DependencyRegistryCommonTestContext) {
          // get config from beforeEach
          const config = this.config;
          await config.dependencyRegistry
            .connect(config.accounts.deployer)
    ...
    describe("adminACLContract", function () {
      it("returns expected adminACLContract address", async function (this: DependencyRegistryCommonTestContext) {
        const config = await loadFixture(_beforeEach);
      ...
      it("behaves as expected when transferring ownership", async function (this: DependencyRegistryCommonTestContext) {
        const config = await loadFixture(_beforeEach);
        // deploy new ACL with user as superAdmin
        const userAdminACLFactory = new AdminACLV0__factory(
      ...
      it("behaves as expected when renouncing ownership", async function (this: DependencyRegistryCommonTestContext) {
        const config = await loadFixture(_beforeEach);
