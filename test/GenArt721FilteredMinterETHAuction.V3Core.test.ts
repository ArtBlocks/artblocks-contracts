...
describe("GenArt721MinterEthAuction_V3Core", async function () {
  ...
  beforeEach(async function () {
    ...
    // offset 10 days to avoid winding back time after V1Core test completed
    if (this.hasOwnProperty("startTime") && this.startTime) {
      this.startTime = this.startTime + ONE_DAY * 10;
    } else {
      this.startTime = Date.now() + ONE_DAY * 10;
    }
