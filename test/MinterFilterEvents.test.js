...
describe('MinterFilterEvents', async function() {
    ...
    beforeEach(async function() {
        ...
        this.token = await artblocksFactory.connect(snowfro).deploy("Test Contract", "TEST", this.randomizer.address)
    ...
    describe('setDefaultMinter', async function() {
        it('emits an event', async function() {
            await expect(
              this.minterFilter.connect(this.accounts.snowfro).setDefaultMinter(this.minter.address)
            )
    ...
    describe('setMinterForProject', async function() {
        it('emits an event', async function() {
            await expect(
              this.minterFilter.connect(this.accounts.snowfro).setMinterForProject(0, this.minter.address)
            )
            .to.emit(this.minterFilter, "ProjectMinterRegistered")
            .withArgs(0, this.minter.address);

            await expect(
              this.minterFilter.connect(this.accounts.snowfro).setMinterForProject(1, this.minter.address)
            )
            .to.emit(this.minterFilter, "ProjectMinterRegistered")
